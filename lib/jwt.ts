...
export class JWTService{
    private static ACCESS_TOKEN_EXPIRY = process.env.ACCESS_TOKEN_EXPIRY as string;
    private static REFRESH_TOKEN_EXPIRY = process.env.REFRESH_TOKEN_EXPIRY as string;
    private static JWT_SECRET = process.env.JWT_SECRET as string;
    private static REFRESH_TOKEN_SECRET = process.env.JWT_REFRESH_SECRET as string;
    ...
    static async verifyToken(token: string, type: "access" | "refresh"): Promise<TokenPayload | null>{
        try {
            const secret = type === "access" ? this.JWT_SECRET : this.REFRESH_TOKEN_SECRET;
